...
test('POM - Reusing Base Component', async ({ page }) => {
    ...
    const infomationColumnText = await infomationColumnComp.getTitleText();
    const infomationColumnLink = await infomationColumnComp.getLinkLists();
    console.log(`Text is ${infomationColumnText} and Link include [${infomationColumnLink}]`);
    const customServiceColumnText = await custerServiceColumnComp.getTitleText();
    const customServiceColumnLink = await custerServiceColumnComp.getLinkLists();
    console.log(`Text is ${customServiceColumnText} and Link include [${customServiceColumnLink}]`);
    const myAccountColumnText = await myAccountColumnComp.getTitleText();
    const myAccountColumnLink = await myAccountColumnComp.getLinkLists();
